...
now(function()
	...
	-- LazyVim-style per-directory sessions: automatically save a local
	-- 'Session.vim' in the current working directory when quitting Neovim, so
	-- `<Leader>ss` always has something to restore. `autowrite = true` (the
	-- default) only rewrites an *already active* session, so we create the local
	-- one here on exit.
	--
	...
	-- avoids littering directories with sessions for one-off file edits.
	...
	Config.new_autocmd("VimLeavePre", "*", function()
		...
		-- `force = true` overwrites the existing local session without prompting.
		-- Default `session_name` writes the local `config.file` ('Session.vim') in
		-- the current working directory (see `:h MiniSessions.write()`).
		pcall(MiniSessions.write, MiniSessions.config.file, { force = true })
	end, "Auto-save local session on exit")
end)
