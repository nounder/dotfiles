 ...
 *
 * @see https://github.com/mitsuhiko/agent-stuff/blob/main/pi-extensions/files.ts
 */
...
> => {
  ...
  const untrackedResult = await pi.exec("git", [
    "ls-files",
    "-z",
    "--others",
    "--exclude-standard",
...
const buildFileEntries = async (
  ...
  const upsertFile = (
    ...
    const displayPath = data.displayPath
      ?? formatDisplayPath(data.canonicalPath, ctx.cwd)
    ...
    if (existing) {
      fileMap.set(data.canonicalPath, {
        ...
        hasSessionChange: existing.hasSessionChange
          || data.hasSessionChange === true,
...
const revealPath = async (
  ...
  const isDirectory = target.isDirectory
    || statSync(target.resolvedPath).isDirectory()
  let command = "open"
  let args: string[] = []
...
const openDiff = async (
  ...
  const existsInHead = await pi.exec("git", [
    "cat-file",
    "-e",
    `HEAD:${relativePath}`,
  ], { cwd: gitRoot })
  if (existsInHead.code === 0) {
    ...
    if (result.code !== 0) {
      const errorMessage = result.stderr?.trim()
        || `Failed to diff ${target.displayPath}`
      ctx.ui.notify(errorMessage, "error")
      return
    }
  ...
  if (openResult.code !== 0) {
    const errorMessage = openResult.stderr?.trim()
          ...
          if (matchesKey(data, "ctrl+shift+d")) {
            ...
            if (selected) {
              const file = files.find((entry) =>
                entry.canonicalPath === selected.value
              )
              const canDiff = file?.isTracked && !file
                .isDirectory && Boolean(gitRoot)
              if (!canDiff) {
                ctx
                  .ui
                  .notify("Diff is only available for tracked files", "warning")
                return
              }
              quickAction = "diff"
              done(
...
const runFileBrowser = async (
  ...
  while (true) {
    ...
    const canDiff = selected.isTracked
