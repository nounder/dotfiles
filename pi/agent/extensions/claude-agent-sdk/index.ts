...
const GLOBAL_SKILLS_ROOT = NPath.join(NOs.homedir(), ".pi", "agent", "skills")
const PROJECT_SKILLS_ROOT = NPath.join(process.cwd(), ".pi", "skills")
const GLOBAL_SETTINGS_PATH = NPath.join(NOs.homedir(), ".pi", "agent", "settings.json")
const PROJECT_SETTINGS_PATH = NPath.join(process.cwd(), ".pi", "settings.json")
const GLOBAL_AGENTS_PATH = NPath.join(NOs.homedir(), ".pi", "agent", "AGENTS.md")
...
function extractToolExecutionContent(result: unknown): string {
  if (result && typeof result === "object" && "content" in result && Array.isArray(result.content)) {
    const text = contentToPlainText(result.content)
    if (text) return truncateText(text, MAX_TRACKED_TOOL_CONTENT_CHARS)
  }
  return truncateText(stringifyUnknown(result), MAX_TRACKED_TOOL_CONTENT_CHARS)
}
...
async function* buildPromptStream(promptBlocks: ContentBlockParam[]): AsyncIterable<SDKUserMessage> {
  yield {
    type: "user",
    message: {
      role: "user",
...
function extractSkillsAppend(systemPrompt?: string): string | undefined {
  ...
  return rewriteSkillsLocations(systemPrompt.slice(startIndex, endIndex + endMarker.length).trim())
}
...
function readSettingsFile(filePath: string): ProviderSettings {
  if (!NFs.existsSync(filePath)) return {}
  try {
    const raw = NFs.readFileSync(filePath, "utf-8")
...
function rewriteSkillsLocations(skillsBlock: string): string {
  return skillsBlock.replace(/<location>([^<]+)<\/location>/g, (_match, location: string) => {
    ...
    if (location.startsWith(GLOBAL_SKILLS_ROOT)) {
      const relPath = NPath.relative(GLOBAL_SKILLS_ROOT, location).replace(/^\.+/, "")
      rewritten = `${SKILLS_ALIAS_GLOBAL}/${relPath}`.replace(/\/\/+/, "/")
    } else if (location.startsWith(PROJECT_SKILLS_ROOT)) {
      const relPath = NPath.relative(PROJECT_SKILLS_ROOT, location).replace(/^\.+/, "")
      rewritten = `${SKILLS_ALIAS_PROJECT}/${relPath}`.replace(/\/\/+/, "/")
    }
...
function resolveAgentsMdPath(): string | undefined {
  ...
  if (NFs.existsSync(GLOBAL_AGENTS_PATH)) return GLOBAL_AGENTS_PATH
  return undefined
}

function findAgentsMdInParents(startDir: string): string | undefined {
  let current = NPath.resolve(startDir)
  while (true) {
    const candidate = NPath.join(current, "AGENTS.md")
    if (NFs.existsSync(candidate)) return candidate
    const parent = NPath.dirname(current)
...
function extractAgentsAppend(): string | undefined {
  ...
  try {
    const content = NFs.readFileSync(agentsPath, "utf-8").trim()
...
function rewriteSkillAliasPath(pathValue: unknown): unknown {
  ...
  const projectAliasAbs = NPath.join(process.cwd(), SKILLS_ALIAS_PROJECT)
...
function streamClaudeAgentSdk(
  ...
  void (async () => {
    ...
    const findBlock = (eventIndex: number) => {
      const index = blocks.findIndex((block) => block.index === eventIndex)
    ...
    try {
      ...
      if (sessionKey) reconcileToolWatchStateWithContext(sessionKey, context)
      ...
      for await (const message of sdkQuery) {
        ...
        if (message.type === "auth_status") {
          const authMsg = message as { error?: string; output?: string[] }
          if (authMsg.error) {
            throw new Error(authMsg.error)
          }
          continue
        }
        ...
        switch (message.type) {
          case "stream_event": {
            ...
            if (event?.type === "content_block_delta") {
              const { index, block } = findBlock(event.index)
            ...
            if (event?.type === "content_block_stop") {
              ...
              if (block.type === "text") {
                ...
                stream.push({ type: "toolcall_end", contentIndex: index, toolCall: block as any, partial: output })
              }
              break
            }
          ...
          case "result": {
            const resultMsg = message as { subtype?: string; result?: string; errors?: string[]; is_error?: boolean }
            if (resultMsg.subtype !== "success") {
              const detail = resultMsg.errors?.join("; ") || resultMsg.result || "Claude SDK request failed"
              throw new Error(detail)
            }
            if (!sawStreamEvent) {
              output.content.push({ type: "text", text: resultMsg.result || "" } as any)
            }
            break
          }
        }
      ...
      if (wasAborted || options?.signal?.aborted) {
        output.stopReason = "aborted" as any
      ...
      output.stopReason = (options?.signal?.aborted ? "aborted" : "error") as any
